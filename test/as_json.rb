...
test "set a block to return json" do
  ...
  Cuba.define do
    on "drink" do
      as_json 201, {"Content-Location" => "http://somewhere.com/drink/42"} do
        rum_and_coke
      end
    end
  end

  env = { "SCRIPT_NAME" => "/", "PATH_INFO" => "/drink" }
  ...
  assert_equal "http://somewhere.com/drink/42", resp.headers["Content-Location"]
end
